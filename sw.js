...
function addCoopHeaders(response, request) {
    // 🔧 v10.2: только same-origin — не ломаем CDN
    if (request && new URL(request.url).origin !== location.origin) return response;
...
self.addEventListener('fetch', function(e) {
    ...
    if (e.request.url.indexOf('deepseek.com') !== -1 ||
        ...
        return e.respondWith(fetch(e.request).then(function(r) {
            return addCoopHeaders(r, e.request);
        }).catch(function() {
        ...
        caches.match(e.request).then(function(cached) {
            ...
            if (e.request.mode === 'navigate') {
                return caches.match('./index.html').then(function(html) {
                    if (html) return addCoopHeaders(html, e.request);
                    return fetch(e.request).then(function(r){ return addCoopHeaders(r, e.request); });
                });
            }
            ...
            return fetch(e.request).then(function(response) {
                ...
                return addCoopHeaders(response, e.request);
            }).catch(function() {
                return new Response(
